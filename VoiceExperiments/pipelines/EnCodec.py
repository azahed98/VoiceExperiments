...
class EnCodec(nn.Module):
    ...
    def encode_verbose(self, x: torch.Tensor) -> tp.Tuple[tp.List[EncodedFrame], torch.Tensor]:
        ...
        segment_length = self.encodec.segment_length
        if segment_length is None:
            segment_length = length
            stride = length
        else:
            stride = self.encodec.segment_stride  # type: ignore
    ...
    def _encode_frame_verbose(self, x: torch.Tensor) -> EncodedFrame:
        length = x.shape[-1]
        duration = length / self.encodec.sample_rate
        assert self.encodec.segment is None or duration <= 1e-5 + self.encodec.segment
        ...
        quantized_result = self.encodec.quantizer(emb, self.encodec.frame_rate, self.encodec.bandwidth)
    ...
    def gen_step(self, x, lengths_x):
        ...
        G_x = self.encodec.decode(frames)[:, :, :x.shape[-1]]
